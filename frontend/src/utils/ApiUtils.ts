...
import type { FamUserRoleAssignmentCreateResponse } from "fam-api";
...
/**
 * Groups an array of FamUserRoleAssignmentCreateResponse items by user ID.
 * @param items An array of FamUserRoleAssignmentCreateResponse items to be grouped by user ID.
 * @returns A map where the key is the user ID and the value is an array of
 *          FamUserRoleAssignmentCreateResponse items for that user.
 */
export const mapAppUserGrantResponseByUserId = (
    items: Array<FamUserRoleAssignmentCreateResponse>
): Map<number, FamUserRoleAssignmentCreateResponse[]> => {
    return items.reduce((map, item) => {
        ...
        return map;
    }, new Map<number, FamUserRoleAssignmentCreateResponse[]>());
};