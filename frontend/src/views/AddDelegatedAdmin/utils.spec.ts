...
const role = (name: string, district = false, client = false) =>
    ({
        name,
        display_name: null,
        description: null,
...
const selection = (
    ...
    districts: any[] = [],
    forestClients: any[] = []
) => ({ ...newRoleSelection(r), districts, forestClients });
...
describe("requiresScope", () => {
    ...
    it("is true when the role is scoped either way", () => {
...
describe("newRoleSelection", () => {
    it("gives each role its own search box id", () => {
        // Two client-scoped roles on screen at once would otherwise share an id,
        // breaking the label association for both and echoing each other's
        // typing.
        expect(newRoleSelection(role("A")).forestClientInput.id).not.toBe(
            newRoleSelection(role("B")).forestClientInput.id
        );
    });
