    ...
    queryFn: () =>
        AppActlApiService.applicationsApi
            .getFamApplicationUserRoleAssignment(
                ...
                // null means "no search"; the client omits an undefined param.
                backendPagination.value.search ?? undefined,
                backendPagination.value.sortOrder ?? undefined,
                backendPagination.value.sortBy ?? undefined
            )
            .then((res) => res.data),
    refetchOnMount: "always",
...
const deleteAppUserRoleMutation = useMutation({
    mutationFn: (appUser: FamApplicationUserRoleAssignmentGetDto) =>
...
const handleDelete = (privilegeObject: TableRowType) => {
    ...
    if (isAppUserTable) {
        const appUser =
            privilegeObject as FamApplicationUserRoleAssignmentGetDto;
        setConfirmTextProps(
            userName,
...
const highlightNewUserAccessRow = (
    ...
    switch (props.tableType) {
        ...
        case ManagePermissionsTableEnum.AppUser:
            const appAdmin =
                rowData as FamApplicationUserRoleAssignmentGetDto;
...
const handleSort = (event: DataTableSortEvent): void => {
    ...
    if (sortOrder === 0) {
        ...
        backendPagination.value.sortOrder =
            sortOrder === 1 ? SortOrder.Asc : SortOrder.Desc;
