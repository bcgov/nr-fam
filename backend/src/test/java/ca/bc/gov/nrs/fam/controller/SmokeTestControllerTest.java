 ...
 *
 * <p>It reads reference data rather than user data. Counting a user table
...
class SmokeTestControllerTest {
  ...
  @Mock private FamPrivilegeChangeTypeRepository changeTypeRepository;
  ...
  void okWhenMigrated() {
    ...
    when(changeTypeRepository.count()).thenReturn(5L);
  ...
  void okOnAFreshlyDeployedEnvironment() {
    ...
    // Nothing here consults user data at all, which is the point.
    when(changeTypeRepository.count()).thenReturn(5L);
  ...
  void expectationFailedWhenUnmigrated() {
    ...
    when(changeTypeRepository.count()).thenReturn(0L);
